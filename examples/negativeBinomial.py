"""Negative Binomial example for plotcdf.
...
def main() -> None:
	...
	negative_binomial_from_pmf = plot.RV(support=support, prob=prob, complete_right=False)
	negative_binomial_rv = plot.RV(support=support, prob=prob_sum, is_pmf=False, complete_right=False)

	fig_cdf, _ = negative_binomial_rv.plot_cdf(rv_name=rv_name, save=False)
	fig_cdf.savefig(os.path.join(output_folder, f"{rv_name}_cdf.png"), dpi=300)

	fig_pmf, _ = negative_binomial_from_pmf.plot_pmf(rv_name=rv_name, save=False)
	fig_pmf.savefig(os.path.join(output_folder, f"{rv_name}_pmf.png"), dpi=300)

	fig_qt, _ = negative_binomial_rv.plot_quantile(rv_name=rv_name, save=False)
	...
	negative_binomial_from_pmf = plot.RV(
		support=support,
		prob=prob,
		complete_left=False,
